...
\subsubsection{Code} The diffusion term is easily accomplished in the following few lines of code: 
...
if((x >= 1) && (x <= x_max - 1)){
    x_diffusion = (1/R)*(vec_time[time][y][x + 1] - (2 * vec_time[time][y][x])
    + vec_time[time][y][x - 1]);
}
if((y >= 1) && (y <= y_max - 1)){
    y_diffusion = (1/R)*(vec_time[time][y + 1][x] - (2 * vec_time[time][y][x]) 
    + vec_time[time][y - 1][x]);
}
...
The above code is broken into two likes due to line length. And for the edges, one can do something like this:
...
In this case, $R$ is more similar to the resistance to movement of particles. It is kind of like the inverse of the diffusion coefficient. Whatever you choose to do is fine, though. 

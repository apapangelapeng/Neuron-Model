...
\subsection{Open and Closed Calculating} For the moment, I have three states: defined here as $N_{open}$, $N_{inactive}$, $N_{closed}$. $N_{inactive}$ meaning that the channels are not open, but still cannot be opened for some time. The calculations done for each, thus, are: 

\bigskip

\begin{center}

$N_{open}^{i+1} = P_TN_{closed}^{i}$ \\ 
\smallskip
$N_{inactive}^{i+1} = e^{\Delta t / \tau}N_{inactive}^{i} + (N_{open}^{i} - N_{open}^{i}e^{\Delta t / \tau})$ \\ 
\smallskip
$N_{closed}^{i+1} = (1 - P_T)N_{closed}^{i} + (N_{inactive}^{i} - N_{inactive}^{i}e^{\Delta t / \tau})$ \\

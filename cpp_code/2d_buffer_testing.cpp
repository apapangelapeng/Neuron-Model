    ...
    for(int time_temp = 0; time_temp <= time_max; time_temp++){
        for(int i = 0; i <= x_max; i++){
            for(int j = 0; j <= y_max; j++){
                ...
                // + scaling_factor*Compute_J_on(C_cyt, time_temp, i, j)
                Ca_c_dT = delta_T*(scaling_factor*Piezo_Channel(E_Ca, time_temp, i, j) + scaling_factor*Compute_J_diffusion(time_temp, j, i) + scaling_factor*Compute_J_on(C_cyt, time_temp, i, j) + scaling_factor*Compute_J_ryr(C_cyt, time_temp, i, j) - scaling_factor*Compute_J_serca(C_cyt, time_temp, i, j));
